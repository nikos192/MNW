...
const ScrollExpandMedia = ({
  ...
  const mediaWidth = 36 + scrollProgress * (isMobileState ? 56 : 58);
  const mediaHeight = 42 + scrollProgress * (isMobileState ? 30 : 42);
  const textTranslateX = scrollProgress * (isMobileState ? 7 : 9);
  const headingOpacity = 1 - scrollProgress * 0.62;
  const showContent = scrollProgress > 0.45;
  ...
  return (
    <div
      ref={sectionRef}
      className='relative min-h-[180svh] overflow-x-hidden bg-[#111110] text-[#f5f5f3]'
    >
      <section className='sticky top-0 flex min-h-[100dvh] flex-col items-center justify-start overflow-hidden'>
        <div className='relative flex min-h-[100dvh] w-full flex-col items-center'>
          ...
          <div className='container relative z-10 mx-auto flex h-[100dvh] flex-col items-center justify-center'>
            ...
            <motion.section
              className='pointer-events-none absolute inset-x-0 bottom-[8vh] mx-auto flex w-full max-w-6xl flex-col px-6 md:px-10 lg:px-16'
              initial={{ opacity: 0, y: 32 }}
              animate={{ opacity: showContent ? 1 : 0, y: showContent ? 0 : 32 }}
              transition={{ duration: 0.45 }}
            >
              <div className='pointer-events-auto rounded-[32px] border border-white/10 bg-[rgba(16,16,16,0.78)] p-8 backdrop-blur-xl md:p-10'>
                {children}
              </div>
            </motion.section>
          </div>
